    ...
    <div class="bwg_image_wrap">
      ...
      <div class="bwg_btn_container">
        <div class="bwg_ctrl_btn_container">
          ...
          <i title="<?php echo __('Show comments', 'bwg'); ?>" class="bwg_ctrl_btn bwg_comment fa fa-comment"></i>
          <?php }?>
          <?php
          if ($enable_image_facebook) {
            ?>
            <a id="bwg_facebook_a" href="https://www.facebook.com/sharer/sharer.php?s=100&p[url]=<?php echo urlencode($current_url); ?>&p[title]=<?php echo $current_image_alt; ?>&p[summary]=<?php echo $current_image_description; ?>&p[images][0]=<?php echo urlencode(site_url() . '/' . $WD_BWG_UPLOAD_DIR . '/thumb' . $current_image_url); ?>" target="_blank" title="<?php echo __('Share on Facebook', 'bwg'); ?>">
              <i title="<?php echo __('Share on Facebook', 'bwg'); ?>" class="bwg_ctrl_btn bwg_facebook fa fa-facebook"></i>
            </a>
            <?php
          }
          if ($enable_image_twitter) {
            ...
            <a href="https://twitter.com/share?url=<?php echo urlencode($current_url); ?>" target="_blank" title="<?php echo __('Share on Twitter', 'bwg'); ?>">
              <i title="<?php echo __('Share on Twitter', 'bwg'); ?>" class="bwg_ctrl_btn bwg_twitter fa fa-twitter"></i>
            </a>
            <?php
          }
          if ($enable_image_google) {
            ?>
            <a href="https://plus.google.com/share?url=<?php echo urlencode($current_url); ?>" target="_blank" title="<?php echo __('Share on Google+', 'bwg'); ?>">
      ...
      if ($enable_image_filmstrip) {
        ...
        <div class="bwg_filmstrip_container">
          <div class="bwg_filmstrip_left"><i class="fa fa-angle-left "></i></div>
          <div class="bwg_filmstrip">
            <div class="bwg_filmstrip_thumbnails">
              <?php
              foreach ($image_rows as $key => $image_row) {
                if ($image_row->id == $current_image_id) {
                  $current_pos = $key * ($image_filmstrip_width + $filmstrip_thumb_margin_hor);
                  $current_key = $key;
                }
              ...
              <div id="bwg_filmstrip_thumbnail_<?php echo $key; ?>" class="bwg_filmstrip_thumbnail <?php echo (($image_row->id == $current_image_id) ? 'bwg_thumb_active' : 'bwg_thumb_deactive'); ?>">
                <img style="width:<?php echo $image_thumb_width; ?>px; height:<?php echo $image_thumb_height; ?>px; margin-left: <?php echo $thumb_left; ?>px; margin-top: <?php echo $thumb_top; ?>px;" class="bwg_filmstrip_thumbnail_img" src="<?php echo ($is_video ? "" : site_url() . '/' . $WD_BWG_UPLOAD_DIR) . $image_row->thumb_url; ?>" onclick="bwg_change_image(parseInt(jQuery('#bwg_current_image_key').val()), '<?php echo $key; ?>', data)" image_id="<?php echo $image_row->id; ?>" image_key="<?php echo $key; ?>" alt="<?php echo $image_row->alt; ?>" />
              </div>
              <?php
              }
              ?>
          ...
          <div class="bwg_filmstrip_right"><i class="fa fa-angle-right"></i></div>
        </div>
        <?php
      }
      if ($watermark_type != 'none') {
    ...
    <div class="bwg_comment_wrap">
      <div class="bwg_comment_container bwg_close">
        ...
        <div class="bwg_comments">
          ...
          if (isset($_POST['ajax_task']) && (esc_html(stripslashes($_POST['ajax_task'])) === 'save')) {
            if ($option_row->popup_enable_captcha) {
              ...
              if ($bwg_captcha_input !== $bwg_captcha_code) {
                $captcha_error_message = __('Error. Incorrect Verification Code.', 'bwg');
            ...
            if ($option_row->popup_enable_email && isset($_POST['bwg_email']) && !is_email(stripslashes($_POST['bwg_email']))) {
              $email_error_message = __( 'This is not a valid email address.', 'bwg' );
              $bwg_name = (isset($_POST['bwg_name']) ? esc_html(stripslashes($_POST['bwg_name'])) : '');
              $bwg_comment = (isset($_POST['bwg_comment']) ? esc_html(stripslashes($_POST['bwg_comment'])) : '');
              $bwg_email = (isset($_POST['bwg_email']) ? esc_html(stripslashes($_POST['bwg_email'])) : '');
            }
          ...
          ?>
          <div id="bwg_comments">
            <div title="<?php echo __('Hide Comments', 'bwg'); ?>" class="bwg_comments_close">
            ...
            <form id="bwg_comment_form" method="post" action="<?php echo $popup_url; ?>">
              <p><label for="bwg_name"><?php echo __('Name', 'bwg'); ?> * </label></p>
              ...
              ?>
              <p><label for="bwg_email"><?php echo __('Email', 'bwg'); ?> * </label></p>
              ...
              ?>
              <p><label for="bwg_comment"><?php echo __('Comment', 'bwg'); ?> * </label></p>
              <p><textarea class="bwg_comment_textarea" name="bwg_comment" id="bwg_comment"><?php echo $bwg_comment; ?></textarea></p>
              <?php
              if ($option_row->popup_enable_captcha) {
              ?>
              <p><label for="bwg_captcha_input"><?php echo __('Verification Code', 'bwg'); ?></label></p>
              <p>
                <input id="bwg_captcha_input" name="bwg_captcha_input" class="bwg_captcha_input" type="text">
                <img id="bwg_captcha_img" class="bwg_captcha_img" type="captcha" digit="6" src="<?php echo add_query_arg(array('action' => 'bwg_captcha', 'digit' => 6, 'i' => ''), admin_url('admin-ajax.php')); ?>" onclick="bwg_captcha_refresh('bwg_captcha')" />
                <span id="bwg_captcha_refresh" class="bwg_captcha_refresh" onclick="bwg_captcha_refresh('bwg_captcha')"></span>
              </p>
              <p><span class="bwg_comment_error"><?php echo $captcha_error_message; ?></span></p>
              <?php
              }
              ?>
              <p><input onclick="if (spider_check_required('bwg_name', '<?php echo __('Name', 'bwg'); ?>') <?php if ($option_row->popup_enable_email) { ?> || spider_check_required('bwg_email', '<?php echo __('Email', 'bwg'); ?>') || spider_check_email('bwg_email') <?php } ?> || spider_check_required('bwg_comment', '<?php echo __('Comment', 'bwg'); ?>')) {return false;}
                                 var cur_image_key = parseInt(jQuery('#bwg_current_image_key').val());
                                 ++data[cur_image_key]['comment_count'];
                                 spider_set_input_value('ajax_task', 'save');
                                 spider_set_input_value('image_id', jQuery('#bwg_popup_image').attr('image_id'));
                                 spider_ajax_save('bwg_comment_form');
                                 return false;" class="bwg_submit" type="submit" name="bwg_submit" id="bwg_submit" value="<?php echo __('Submit', 'bwg'); ?>" /></p>
                ...
                <p class="bwg_comment_header_p">
                  ...
                  if (current_user_can('manage_options')) {
                    ...
                    <i onclick="var cur_image_key = parseInt(jQuery('#bwg_current_image_key').val());
                           ...
                           spider_ajax_save('bwg_comment_form');
                           return false;" title="<?php echo __('Delete Comment', 'bwg'); ?>" class="bwg_comment_delete_btn fa fa-times"></i>
                    <?php
                  }
                  ?>
