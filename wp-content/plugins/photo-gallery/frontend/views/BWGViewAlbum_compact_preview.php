  ...
  public function display($params, $from_shortcode = 0, $bwg = 0) {
    ...
    if ($album_view_type == 'masonry' && $type == 'gallery' ) {
      $form_child_div_id = 'bwg_masonry_thumbnails_div_' . $bwg;
        $form_child_div_style = 'background-color:rgba(0, 0, 0, 0); text-align:' . $theme_row->masonry_thumb_align . '; width:100%;';
      $album_gallery_div_id = 'bwg_masonry_thumbnails_' . $bwg;
        $album_gallery_div_class = 'bwg_masonry_thumbnails_' . $bwg;
    }
    else {
      $form_child_div_id = '';
      $form_child_div_style = 'background-color:rgba(0, 0, 0, 0); text-align:center; width:100%;';
    }
              ...
              elseif ($type == 'gallery') {
                ...
                foreach ($image_rows as $image_row) {
                  ...
                  <span class="bwg_masonry_thumb_spun_<?php echo $bwg; ?>">
                    <a style="font-size: 0;" onclick="spider_createpopup('<?php echo addslashes(add_query_arg($params_array, admin_url('admin-ajax.php'))); ?>', '<?php echo $bwg; ?>', '<?php echo $params['popup_width']; ?>', '<?php echo $params['popup_height']; ?>', 1, 'testpopup', 5); return false;">
                      <img class="bwg_masonry_thumb_<?php echo $bwg; ?>" id="<?php echo $image_row->id; ?>" src="<?php echo site_url() . '/' . $WD_BWG_UPLOAD_DIR . $image_row->thumb_url; ?>" alt="<?php echo $image_row->alt; ?>" style="max-height: none !important;  max-width: none !important;" />
                    </a>
                  </span>
                    <?php
                  }
            ...
            if ($album_view_type == 'masonry' && $type == 'gallery' ) {
              ...
              <script>
                function bwg_masonry_<?php echo $bwg; ?>() { 
                  var image_width = <?php echo $params['compuct_album_image_thumb_width']; ?>;
                  var cont_div_width = <?php echo $params['compuct_album_image_column_number'] * ($params['compuct_album_image_thumb_width'] + 2 * ($theme_row->thumb_padding + $theme_row->thumb_border_width)); ?>;
