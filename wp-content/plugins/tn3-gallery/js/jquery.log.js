...
(function($){
	...
	var $log;
	$.extend({"logh":function(){ 
		if(arguments.length > 0) {
			
			// join for graceful degregation
			var args = (arguments.length > 1) ? Array.prototype.join.call(arguments, " ") : arguments[0];
			if ($log) {
			    $log.html($log.html() + "<br/>" + args);
			} else {	    
			    $log = $("body").prepend('<div style="position:fixed;width:300px;height:300px;background-color:#ffffff;overflow-y:auto;padding:20px;border:1px solid #000000;z-index:2000">' + args + '</div>').find(":first");
			    $log.click(function(e) {$log.remove()});
			}
			return false;
		}
	}});
